...
void planificador_largo_plazo(int b)
{
    ...
    while(1)
    {
        ...
        log_info(logger_hilo,"Hice wait de la cola de new");
        t_pcb* pcb = queue_pop(cola_new_h);
        sem_post(&mutex_cola_new);
        sem_wait(&mutex_cola_ready);
        queue_push(cola_ready_h, pcb);
        sem_post(&mutex_cola_ready);
        log_info(logger_hilo, "PID: %i - Estado Anterior: NEW - Estado Actual: READY", 1);
        liberar_pcb(pcb);
    }
}
...
int main(int argc, char* argv[]){
    ...
    pthread_create(&hilo_planificador_de_largo_plazo, NULL, &planificador_largo_plazo, (void*)1);
    printf("Creé el hilo\n");
